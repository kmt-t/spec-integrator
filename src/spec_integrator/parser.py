...
class MarkdownParser:
    ...
    def parse_file(self, file_path: Path, docs_root: Path) -> ParsedDocument:
        ...
        if not heading_indices:
            ...
            for i, (line_start, level, heading) in enumerate(heading_indices):
                line_end = (
                    heading_indices[i + 1][0] - 1 if i + 1 < len(heading_indices) else len(lines)
                )
                sec_lines = lines[line_start - 1 : line_end]
                sec = self._create_section(
                    rel_path, heading, level, line_start, line_end, sec_lines
                )
                sections.append(sec)
    ...
    def _find_heading_lines(
        self, lines: list[str], tokens: list[dict]
        ...
        for level, _heading_text in ast_headings:
