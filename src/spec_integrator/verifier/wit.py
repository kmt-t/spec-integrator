...
class WITVerifier:
    ...
    def verify_documents(self, documents: list[ParsedDocument], docs_root: Path) -> tuple[list[VerificationIssue], list[WITFileResult]]:
        ...
        for wit_file in docs_root.rglob("*.wit"):
            ...
            if not any(r.wit_file == rel_p for r in results):
                ...
                comp = wit_file.parent.parent.name if wit_file.parent.name == wit_dir_name else wit_file.parent.name
    ...
    def verify_wit_file(self, wit_path: Path, component: str, docs_root: Path) -> tuple[WITFileResult, list[VerificationIssue]]:
        ...
        details = f"Valid WIT specification ({len(interfaces)} interface(s), {len(worlds)} world(s))"
        return WITFileResult(
            component=component,
            wit_file=rel_path,
            status="PASS",
    ...
    def _validate_wit_syntax(self, content: str) -> tuple[dict | None, list[str], list[str]]:
        ...
        for line_no, line in clean_lines:
            ...
            if line.startswith("package "):
                if not re.match(r"^package\s+([a-z0-9\-]+:[a-z0-9\-]+(@[0-9]+\.[0-9]+\.[0-9]+)?);?$", line.rstrip(";")):
            ...
            for char in line:
                if char in "{(":
                    ...
                    if not brace_stack or brace_stack[-1][0] != "{":
                        return {"line": line_no, "message": f"Mismatched closing brace '}}' at line {line_no}"}, interfaces, worlds
                    brace_stack.pop()
                elif char == ")":
                    if not brace_stack or brace_stack[-1][0] != "(":
                        return {"line": line_no, "message": f"Mismatched closing parenthesis ')' at line {line_no}"}, interfaces, worlds
                    brace_stack.pop()

        if brace_stack:
            unclosed_char, unclosed_line = brace_stack[-1]
            return {"line": unclosed_line, "message": f"Unclosed '{unclosed_char}' starting at line {unclosed_line}"}, interfaces, worlds
