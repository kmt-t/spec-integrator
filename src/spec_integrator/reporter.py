...
class Reporter:
    ...
    def generate_markdown_report(
        ...
        if obligation_summary is not None:
            ...
            lines.append(
                f"| Verification Obligations Discharged | {obligation_summary.discharged} |"
            )
        ...
        if not issues:
            ...
            for issue in issues:
                ...
                lines.append(
                    f"| **{issue.severity}** | {issue.gate} | {loc} | `{issue.rule_code}` | {issue.message} |"
                )
            lines.append("")
        ...
        if formal_results:
            ...
            for r in formal_results:
                ...
                lines.append(
                    f"| `{r.component}` | `{r.model_file}` | {backs} | {st} | {r.details} |"
                )
            lines.append("")
        ...
        if obligation_summary is not None:
            ...
            pct = round(obligation_summary.coverage * 100)
            lines.append(
                f"- Demanded: **{obligation_summary.demanded}** / "
                f"Discharged: **{obligation_summary.discharged}** ({pct}%)"
            )
            if obligation_summary.stale_documents:
                lines.append(
                    f"- ⚠️ Stale assessments: {len(obligation_summary.stale_documents)} document(s)"
                )
            if obligation_summary.unassessed_documents:
                lines.append(
                    f"- ⚠️ Never assessed: {len(obligation_summary.unassessed_documents)} document(s)"
                )
            lines.append("")
        ...
        for sg in subgraphs:
            ...
            def_str = (
                "<br>".join(f"`{d.replace('sec:', '')}`" for d in sg["defined_in"]) or "*(None)*"
            )
            ref_str = (
                "<br>".join(f"`{r.replace('sec:', '')}`" for r in sg["referenced_in"])
                or "*(Unreferenced)*"
            )
            status = (
                "🟢 Satisfied"
                if (sg["defined_in"] and sg["referenced_in"])
                else ("🔴 Unreferenced" if not sg["referenced_in"] else "🔴 Undefined")
            )
