...
""",
    ...
    (docs_dir / "tier2_runtime" / "loader.md").write_text(
        """# Loader
## Details
Runtime details.
""",
...
def test_static_verifier_catches_invalid_mermaid(tmp_path):
    ...
    docs_dir.mkdir()
    cfg = Config()
    (docs_dir / "diagram_broken.md").write_text(
        """# Diagram Doc
```mermaid
graph TB
    subgraph "Layer"
...
""",
    ...
    unavailable = next(i for i in issues if i.rule_code == "FMT-MERMAID-VALIDATOR-UNAVAILABLE")
