...
def test_db_crud():
    ...
    # Insert Section
    db.insert_section("sec:docs/requires/req.md#Intro", "docs/requires/req.md", "Intro", 1, 1, 10, "Body text", "shash")
    secs = db.get_all_sections()
    assert len(secs) == 1
    assert secs[0]["heading"] == "Intro"

    # Insert Keyword Reference
    db.insert_keyword_reference("REQ_001", "docs/requires/req.md", "sec:docs/requires/req.md#Intro", "defines", 1)
